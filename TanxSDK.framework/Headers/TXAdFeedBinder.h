...
#import <TanxSDK/TXAdFeedPlayerView.h>
...
@property(nonatomic, strong) TXAdFeedModel * model DEPRECATED_MSG_ATTRIBUTE("已经废弃，请使用adModel");
 ...
 */
- (TXAdFeedPlayerView *)getVideoAdViewWithFrame:(CGRect)frame playConfig:(TXAdFeedTemplateConfig *)playConfig;
 ...
 */
- (NSError *)bindFeedView:(TXAdFeedView *)feedView
                closeView:(nullable UIView *)closeView
             dislikeViews:(nullable NSArray<UIView *> *)dislikeViews;

/**
 *  支持自渲染feedView设置点击区域：( 需在bindFeedView:closeView:dislikeViews: 之后调用 ，避免出错)
 *      1、传入clickableViews不为空时，只有clickableViews 和 feedView区域可以响应点击进行转化
 ...
 */
- (NSError *)registerFeedViewClickableViews:(NSArray<UIView *> *)clickableViews;
